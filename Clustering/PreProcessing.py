...
def pca(data: np.ndarray, variance: float = 0.95) -> np.ndarray:
    ...
    variance (float): The variance % of the dataset to be maintained. Based on this
        number, the PCA algorithm determines the number of principal components
...
def preprocess(
    ...
    currency (np.ndarray): The matrix containing the annual bilateral exchange rate of
        each country's currency and the US Dollar
    variance (float): The variance % of the dataset to be maintained. Based on this
        number, the PCA algorithm determines the number of principal components

    Returns:
    pd.DataFrame: The dataset where each country is represented by the concatenation
        of the principal components of its three time series (gdp, population and
        currency exchange rate)
    np.ndarray: The scaled version of the dataset
    """
    df = pd.DataFrame({})
